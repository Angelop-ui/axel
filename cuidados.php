<h2>Cuidados del Sistema</h2>

<h3>Materiales Recomendados</h3>
<p>
    <img src="materiales.jpg" alt="Materiales recomendados" style="float:right; max-width:200px; margin-left:15px;">
    Es fundamental construir el sistema con materiales resistentes a la intemperie y a la corrosión. El PVC es ideal para tuberías por su bajo costo y durabilidad, mientras que el aluminio y la lámina galvanizada funcionan bien en techos y canaletas. También pueden utilizarse mallas plásticas o de acero inoxidable para filtros.
</p>
<ul>
    <li>🔧 Elige materiales certificados para uso alimentario si el agua será utilizada para consumo humano.</li>
    <li>🧰 Verifica que las conexiones estén bien selladas para evitar fugas.</li>
    <li>🛠️ Reemplaza piezas deterioradas cada cierto tiempo para evitar contaminación del agua.</li>
</ul>

<div style="clear: both;"></div>

<h3>Condiciones Ambientales</h3>
<p>
    <img src="ambiente.jpg" alt="Condiciones ambientales" style="float:left; max-width:200px; margin-right:15px;">
    Para asegurar la eficiencia del sistema, es importante mantenerlo alejado de fuentes de contaminación como chimeneas, árboles que suelten muchas hojas, zonas con humo o residuos industriales. Estas fuentes pueden ensuciar el techo o canaletas, afectando la calidad del agua recolectada.
</p>
<ul>
    <li>🌳 Limpia el techo y canaletas al menos cada 2 meses o después de lluvias fuertes.</li>
    <li>🚫 Instala rejillas o trampas para evitar el paso de hojas, ramas o animales.</li>
    <li>🧽 Realiza un mantenimiento preventivo antes y después de cada temporada de lluvias.</li>
</ul>

<div style="clear: both;"></div>

<h3>Filtros y Almacenamiento</h3>
<p>
    <img src="filtro.png" alt="Filtros y almacenamiento" style="float:right; max-width:220px; margin-left:15px;">
    Los filtros son esenciales para evitar que sedimentos, hojas o excrementos lleguen al tanque de almacenamiento. Se recomienda usar filtros de primera lluvia (first flush) para desechar los primeros litros de cada lluvia, que suelen estar más contaminados.
</p>
<p>
    El agua recolectada debe almacenarse en tinacos o cisternas limpias y cerradas para evitar el crecimiento de bacterias, la entrada de insectos y la proliferación de algas.
</p>

<div style="clear: both;"></div>

<h3>Revisión y Mantenimiento</h3>
<p>
    <img src="mante.jpg" alt="Mantenimiento del sistema" style="float:left; max-width:200px; margin-right:15px;">
    El sistema de captación no debe dejarse sin supervisión. Es importante revisar periódicamente:
</p>
<ul>
    <li>✅ Que las canaletas estén libres de obstrucciones.</li>
    <li>✅ Que los filtros estén limpios y funcionales.</li>
    <li>✅ Que el tanque esté cerrado y libre de fugas.</li>
    <li>✅ Que el agua no presente malos olores ni coloración extraña.</li>
</ul>

<div style="clear: both;"></div>

<h3>Más información y recursos útiles</h3>
<ul>
    <li><a href="https://islaurbana.org/" target="_blank">Isla Urbana – Captación de agua de lluvia</a></li>
    <li><a href="https://www.gob.mx/conagua" target="_blank">CONAGUA – Comisión Nacional del Agua</a></li>
    <li><a href="https://www.sedema.cdmx.gob.mx/programas/programa/cosecha-de-lluvia" target="_blank">Programa Cosecha de Lluvia (CDMX)</a></li>
    <li><a href="https://ecotec.unam.mx/" target="_blank">Ecoteca UNAM – Tecnologías sustentables</a></li>
    <li><a href="https://www.imta.gob.mx/" target="_blank">IMTA – Instituto Mexicano de Tecnología del Agua</a></li>
</ul>
